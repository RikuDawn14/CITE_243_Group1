...
- Bugs I have found are that when trying to run the program as a whole there is an error causing the modules (only website scanner tested so far) to quit before they can run making the program seem like it is hanging. However, if run in debug with a breakpoint in the module the program functions as expected. Will need farther investigate the cause of this bug.